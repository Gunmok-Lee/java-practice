...
public class Ex02Printf {
	public static void main(String[] args) {
		...
		//D. 10진법 정수를 5자리로 출력하고 오른쪽 정렬하되 왼쪽 빈 공간은 0으로 채워라
		System.out.printf("1-D. %05d\n", number);
		
		//2. 16진법 정수(he'x'abecimal)
		//16진법은 오른쪽부터 16의 0승, 1승, 2승, 순으로 자릿수가 정해지고
		//한자리당 숫자가 16개(0~15까지)들어갈 수 있는 숫자체계이다.
		//10부터 15는A, B, C, D, E, F로 치환된다.
		
		//A. 정수를 16진법으로 표시하고 알파벳이 있으면 소문자로 출력해라
		System.out.printf("2-A. %x\n", number);
		//B. 정수를 16진법으로 표시하고 알파벳이 있으면 대문자로 출력해라
		System.out.printf("2-B. %X\n", number);
		//C. 정수를 16진법으로 표시하고 알파벳이 있으면 대문자로 출력화되
		//   총 8자리로 맞추고 오른쪽 정렬하고 왼쪽 빈공간은 0으로 채워라
		System.out.printf("2-C. %08X\n", number);
		
		// 자릿수 지정과 정렬 방법(아무것도 안쓰면 오른쪽, -는 왼쪽 정렬)
		// 그리고 오른쪽 정렬시 왼쪽 빈 공간에 0을 채우는건 모든것이 다 같다.
		// 자릿수 지정의 경우 만약 출력해야할 내용이 자릿수보다 많으면
		// 자릿수 지정은 무시된다!
		...
		//printf의 장점은 여러가지 %문자를 섞어서 쓸 수 있다는 점이 가장 큰 장점이다
		//당!!!!!!!! %문자의 갯수와 데이터타입이 일채히야한다!!!
