...
// 변수의 선언이란
// 해당 변수 공간에 어떠한 종류의 값이 들어가는지와 그 공간의 이름을 정해주는 것을
// 선언 이라고 한다.
// 할당이란 해당 공간에 선언되어있는 종류의 값을 넣는 것을 할당이라고 한다.

// 변수의 들어갈 수 있는 값의 종류는 크게 2가지가 있다.
// 1. 기본형
...
//       정수란? 소수점이 없는 숫자들
//       byte:  8비트
//              비트란 2진법 자릿수 1개를 뜻한다. 즉 8비트는 8자리 2진법 숫자를 뜻한다.
//              -2의 7승부터 2의 7승 - 1까지 총 256개의 숫자가 들어가 있다.
//              2의 7승 -1까지인 이유는 0이 포함되기 때문!
//       short: 16비트
//              -2의 15승부터 2의 15승 - 1까지
...
//       double: 64비트를 이용하여 실수 표기( 자바에서는 우리가 실수를 적으면
//               자동으로 double로 인식한다.)
//    C) 문자형
//       char: character의 줄임말.
...
public class Ex02Variable {
	public static void main(String[] args) {
		...
		//형변환 (typecasting)
		//형변환이란 내가 입력한 값을 자바에서 다른 종류로 바꿔주는 것을
		//형변환(typecasting)이라고 한다.
		...
		//자바에서 내부적으로 자동으로 변수의 자료형(데이터타입)을 바꿔주는 것을
		//암시적 형변환이라고 한다.
		//암시적 형변환은 정수를 실수로 바꿀때나 혹은
		//더 작은 데이터타입을 더 큰 데이터타입으로 바꿀 때 발생한다.
		
		//현재 number의 데이터타입은 int이다.
		//number를 실수형 변수 공간에 넣을때 암시적 형변환이 발생한다.
		double myDouble = number;
		System.out.println("myDoubled의 현재 값: "+myDouble);
		...
		//byte의 최대 값은 2^7 - 1이기 때문에 127이 된다.
		myByte = 127;
		System.out.println("myByte의 현재값: "+myByte);
		myByte = (byte)130;
		...
		//각 자릿수가 오른쪽부터 10의 0승, 1승, 2승 이런식으로 이루어진다.
		//그렇다면
		// 782는 무슨뜻일까?
		// 2 * 10^0 + 8 * 10^1 + 7 * 10^2 이란 뜻이다.
		
		//마찬가지로 2진법은 각 자리가 0~1까지 2가지 숫자로 이루어져 있고
		//각 자릿수가 오른쪽부터 2의 0승, 1승, 2승... 이런식으로 이루어진다.
		// 1110은 무슨뜻일까?
		// 0 * 2^0 + 1 * 2^1 + 1 * 2^2+ 1 * 2^3 이 된다.
		// 풀어서 쓰면
		// 0 * 1 + 1 * 2 + 1 * 4 + 1 * 8
		// 따라서 14가 된다.
		
		// 그렇다면 8비트 숫자 중에서 가장 큰 숫자는
		// 얼마가 되는가? (단 가장 왼쪽은 0이면 +, 1이면  -인것을 기억하자)
		// 0111 1111 -> 2^7 - 1이니깐 127이 된다.
		// 그러면 127+1은 얼마가 되는가?
		// 128
		// 128은 2^7이니깐
		// 1000 0000이 된다.
		...
		// 위의 경우는 우리가 - 라는 부호가 있으니까 -1이 더 크다 라고 해줄 수 있지만
		// 부호가 없다면?
		// 1000 0000 이 클까 1111 1111이 클까?
		...
		// 오히려 음의 최소값으로 빠지는 것을 overflow라고 한다.
